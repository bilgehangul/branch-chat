...
export function BreadcrumbBar() {
  ...
  const chevron = <span className="mx-2 text-zinc-600">›</span>;
      ...
      {visibleAncestors.map((thread, idx) => {
        ...
        return (
          <span key={thread.id} className="flex items-center">
            <button
              className="text-zinc-400 hover:text-zinc-200 cursor-pointer truncate max-w-[120px]"
            ...
            {isCollapsedInsertPoint && collapsedMiddle.length > 0 && (
              <span className="flex items-center relative">
                <button
                  className="text-zinc-400 hover:text-zinc-200 cursor-pointer px-1"
                  onClick={() => setShowDropdown((prev) => !prev)}
                >
                  ...
                </button>
                {showDropdown && (
                  <div className="absolute top-full left-0 mt-1 bg-zinc-800 rounded shadow-lg z-50 py-1 min-w-[140px]">
                    {collapsedMiddle.map((collapsed) => (
                      <button
                        key={collapsed.id}
                        className="block w-full text-left px-3 py-1 text-zinc-300 hover:bg-zinc-700 truncate"
                        onClick={() => {
